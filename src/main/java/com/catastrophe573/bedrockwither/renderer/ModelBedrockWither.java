...
public class ModelBedrockWither<T extends EntityBedrockWither> extends HierarchicalModel<T>
{
	...
	private static <T extends EntityBedrockWither> void setupHeadRotation(T pWither, ModelPart pPart, int facing)
	{
		pPart.yRot = (pWither.getHeadYRot(facing) - pWither.yBodyRot) * ((float) Math.PI / 180F);
		pPart.xRot = pWither.getHeadXRot(facing) * ((float) Math.PI / 180F);
	}
}