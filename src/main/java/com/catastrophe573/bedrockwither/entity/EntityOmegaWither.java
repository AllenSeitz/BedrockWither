...
public class EntityOmegaWither extends Monster implements PowerableMob, RangedAttackMob
{
	...
	private static final EntityDataAccessor<Integer> DATA_TARGET_B = SynchedEntityData.defineId(EntityBedrockWither.class, EntityDataSerializers.INT);
	private static final EntityDataAccessor<Integer> DATA_TARGET_C = SynchedEntityData.defineId(EntityBedrockWither.class, EntityDataSerializers.INT);
	...
	public EntityOmegaWither(EntityType<? extends EntityBedrockWither> type, Level worldIn)
	{
		super(type, worldIn);
	...
	{
		this.goalSelector.addGoal(0, new EntityOmegaWither.WitherDoNothingGoal());
	...
	protected void customServerAiStep()
	{
		if (this.getInvulnerableTicks() > 0)
		{
			...
			this.bossEvent.setProgress(1.0F - (float) k1 / INVULNERABLE_TICKS);
			if (k1 <= 0)
			{
		...
		else
		{
			...
			for (int i = 1; i < 3; ++i)
			{
				if (this.tickCount >= this.nextHeadUpdate[i - 1])
				{
					...
					this.nextHeadUpdate[i - 1] = this.tickCount + 5 + this.random.nextInt(3);
					...
					if (l1 > 0)
					{
						...
						if (livingentity != null && this.canAttack(livingentity) && !(this.distanceToSqr(livingentity) > 900.0D) && this.hasLineOfSight(livingentity))
						{
							...
							this.nextHeadUpdate[i - 1] = this.tickCount + 40 + this.random.nextInt(20); // original code
							this.idleHeadUpdates[i - 1] = 0;
						}
						else
						{
			...
			// on some ticks break any blocks inside this wither's hitbox
	...
	class WitherDoNothingGoal extends Goal
	{
		...
		{
			return EntityOmegaWither.this.getInvulnerableTicks() > 0;
		}
	}
}
